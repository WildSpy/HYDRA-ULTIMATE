...
> [!IMPORTANT]
> На данный момент полностью готовы, отлажены и стабильно работают плагины **AmneziaWG 2.0** (интегрирован напрямую в ядро Sing-Box через TPROXY), **Mieru**, **AnyTLS**, **TrustTunnel** и **NaiveProxy** (работает через HTTP/3 и QUIC).
...
* **Встроенные списки правил (itdoginfo)**:
  * **РФ-сервисы** (`outside-raw.lst`): Российские ресурсы, доступные только с IP-адресов РФ (полезно при обходе зарубежных ограничений с сервера).
