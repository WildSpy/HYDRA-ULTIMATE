...
| :--- | :--- | :--- |
| **v2.3.3**<br>*(09.07.2026)* | - Исправлены параметры ссылок подключения AmneziaWG в подписках (`sn://awg` — корректная сериализация H1–H4 и I1).<br>- Исправлена и доработана обфускация трафика в Mieru (поддержка пресетов `disabled`, `basic`, `medium`, `aggressive`). | - **AmneziaWG: пресеты обфускации под операторов** — добавлен набор готовых carrier-пресетов (`default`, `mobile`, `tele2`, `yota`, `megafon`, `beeline`, `tattelecom`) с оптимальными параметрами Jc/Jmin/Jmax/I1 для каждого оператора.<br>- **Мульти-профильный режим AmneziaWG** — одновременная работа двух интерфейсов: `awg0` (Desktop, подсеть `10.67.67.0/24`) и `awg1` (Mobile, подсеть `10.68.68.0/24`) с раздельными ключами и конфигами для каждого пользователя.<br>- **Авторотация параметров обфускации** — периодическое обновление Jc/Jmin/Jmax/I1 без разрыва туннеля через `awg syncconf`.<br>- **Hardware-tuning VPS** — автоматическая оптимизация sysctl (BBR, IP-forwarding, буферы сокетов), swap и NIC offload (GRO/GSO/TSO) при включении плагина.<br>- Встроенная TUI-панель управления пресетами обфускации Mieru с авто-применением настроек.<br>- Добавлена генерация ссылок `mierus://` с query-параметром `traffic-pattern` для Karing. |
